...
describe('Get ListActivities Endpoint', () => {
  ...
  describe('GET /activities', () => {
    it('should fetch healthy food list successfully', async () => {
      ...
      const { data, error, status } = await api.activities.get({
        $headers: {
      ...
      expect(data).toHaveProperty('message', 'Activities fetched successfully');
    ...
    it('should fail to fetch healthy food list without token', async () => {
      const { data, status } = await api.activities.get({
        // @ts-ignore ts-expect-error
        $query: {},
        $headers: {},
      });
